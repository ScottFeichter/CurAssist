...
/**
 * Sanitizes a service short description from a spreadsheet.
...
export function sanitizeIncoming(value: any): SanitizeResult {
  ...
  if (cleaned.length > constraints.maxLength) {
    return { valid: false, value: cleaned, errors: [`Short description exceeds ${constraints.maxLength} character limit (${cleaned.length} characters)`] };
  }
...
/**
 * Sanitizes a short description imported from the SFSG API.
 * Trusts SFSG as-is — no modification.
 *
 * @param value - Short description from SFSG API response
 */
...
/**
 * Prepares short description for SFSG API.
 * Strips HTML and trims. SFSG expects plain text.
 *
 * @param value - Short description from MongoDB
 */
