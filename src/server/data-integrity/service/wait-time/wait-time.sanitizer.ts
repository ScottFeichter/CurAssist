...
/**
 * Sanitizes service wait time from a spreadsheet.
...
export function sanitizeIncoming(value: any): SanitizeResult {
  ...
  if (cleaned.length > constraints.maxLength) {
    return { valid: false, value: cleaned, errors: [`Wait time exceeds ${constraints.maxLength} character limit (${cleaned.length} characters)`] };
  }
...
/**
 * Sanitizes wait time imported from the SFSG API.
 * Trusts SFSG as-is — no modification.
 *
 * @param value - Wait time from SFSG API response
 */
...
/**
 * Prepares wait time for SFSG API.
 * Strips HTML and trims. SFSG expects plain text.
 *
 * @param value - Wait time from MongoDB
 */
