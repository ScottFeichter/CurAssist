...
/**
 * Sanitizes service application process from a spreadsheet.
...
export function sanitizeIncoming(value: any): SanitizeResult {
  ...
  if (cleaned.length > constraints.maxLength) {
    return { valid: false, value: cleaned, errors: [`Application process exceeds ${constraints.maxLength} character limit (${cleaned.length} characters)`] };
  }
...
/**
 * Sanitizes application process imported from the SFSG API.
 * Trusts SFSG as-is — no modification.
 *
 * @param value - Application process from SFSG API response
 */
...
/**
 * Prepares application process for SFSG API.
 * Strips HTML and trims. SFSG expects plain text.
 *
 * @param value - Application process from MongoDB
 */
